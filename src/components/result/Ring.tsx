...
export function Ring({ label, value, estimated }: RingProps) {
  ...
  const accent = estimated ? "hsl(var(--accent-primary) / 0.6)" : "hsl(var(--accent-primary))";
  const track = "hsl(var(--surface-elevated) / 0.25)";
  ...
  return (
    <div className="flex flex-col items-center gap-2 text-theme-secondary">
      <div
        ...
        title={`${label}: ${v ?? "–"} / 5`}
      >
        <div className="absolute inset-[4px] rounded-full bg-[hsl(var(--bg-surface)/0.75)] backdrop-blur" />
