...
function fillMissingFields(finalData: any, webData: any, ocrText: string, allowHeuristics = true) {
  ...
  const defaults = allowHeuristics
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { imageBase64, allowHeuristics } = reqJson ?? {};
...
<<<${JSON.stringify(WEB_JSON)}>>>
    ...
    finalData = fillMissingFields(finalData, WEB_JSON, ocrText, allowHeuristics !== false);
